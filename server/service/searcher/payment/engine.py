...
class Search_Engine(Base):
    ...
    def __Select_Payment(self):
        ...
        self.payments = SQL.Filter(Model_Payment, None,
            ...
            reduce(operator.or_, (Q(model_delivery_address__name__icontains=s)          for s in self.words)) |
            reduce(operator.or_, (Q(model_delivery_address__surname__icontains=s)       for s in self.words)) |
            reduce(operator.or_, (Q(model_delivery_address__company_name__icontains=s)  for s in self.words)) |
            reduce(operator.or_, (Q(model_delivery_address__nip__icontains=s)           for s in self.words)) |
            ...
            reduce(operator.or_, (Q(model_invoice_address__name__icontains=s) for s in self.words)) |
            reduce(operator.or_, (Q(model_invoice_address__surname__icontains=s) for s in self.words)) |
            reduce(operator.or_, (Q(model_invoice_address__company_name__icontains=s) for s in self.words)) |
            reduce(operator.or_, (Q(model_invoice_address__nip__icontains=s) for s in self.words)) |
